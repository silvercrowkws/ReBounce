...
public enum CardEffectType
{
    ...
    ModifiedBullets,            // 탄환 개조 : 모든 공 공격력 +2
    ...
    StrongBurn,                 // 강한 화상 : 화상 피해 +2
    ResidualFire,               // 잔불 : 화상 지속 시간 +1초
    Ignition,                   // 점화 : 화상 상태의 대상에게 가하는 피해 +20%

    WaterAccumulation,          // 수분 축적 : 물 공 공격력 +5
    Cooling,                    // 냉각 : 젖음 상태의 적이 받는 피해 +10%
    PurifyingWater,             // 정화수 : 물 공 적중 시 젖음 지속시간 +5초

    Shatter,                    // 파쇄 : 땅 공 공격력 +5
    Crush,                      // 압괴 : 체력이 50% 미만인 적에게 땅 공 피해 +50%
    Crack,                      // 균열 : 땅 공의 추가 피해 배율 +20%

    AmplificationCircuit,       // 증폭 회로 : 전기 공 공격력 +5
    Overcurrent,                // 과전류 : 전이 피해 +10%
    VoltageFocus,               // 전압 집중 : 전기 공의 직접 피해 +20%
    ...
    Pulverize,                  // 분쇄 : 땅 공의 추가 피해 배율 +50%

    Superconductivity,          // 초전도 : 전기 공 공격력 +10
    LightningStrike,            // 낙뢰 : 전기 공의 직접 피해 +50%
    ExtendedCircuit,            // 확장 회로 : 전이 범위 +1(중접X)

    Storm,                      // 폭풍 : 바람 공 공격력 +10
    ...
    Thunderburst,               // 뇌폭 : 전이 피해가 직접 피해와 동일해짐
